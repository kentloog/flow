...
Chart reads `.flow/config.yml` like any other subcommand (a first-ever `/flow` command can land here - bootstrap via the setup step if the config is missing); it needs at least `layout`/`repos` for the research fan-out.
...
- **Research** (AFK): surface a fact a decision waits on. Investigate codebase questions per repo and provide a `path:line` anchor per claim. Investigate external questions from primary sources and provide a URL per claim. Run independent investigations concurrently when the harness supports it and delegation is useful. Distill the answer into the ticket's `## Answer`; park long raw notes in `assets/` and link them. The Answer must stand alone without the asset - `assets/` is deleted when the way is clear, so an answer that only makes sense with its raw notes is under-distilled.
...
- **Grilling** (HITL, the default): conversation per `grill-discipline.md` (this directory) - short rounds of independent questions with recommended answers, the five challenge triggers, concrete-scenario stress tests. Decisions land in the ticket's Answer on resolution.
...
5. **Run the research tickets**: resolve independent research tickets concurrently when the harness supports it, recording answers and Decisions-so-far pointers like any resolution.
