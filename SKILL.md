...
| Repos | single-repo: the project root IS the repo (default). Multi-repo: config lists the repo subdirectories. A repo name is a state.yaml field, never invented as a path segment |
...
Read `state.yaml` first on every subcommand - except `setup`, `chart` (maps have no state.yaml; map.md is their state) and `list`. On every update: bump `updated`, set `current` to the running subcommand, flip `steps.<subcommand>` (`done` on success; qa only when a pass ends PASS), plus the step file's stated writes. Open `state-schema.md` (the full contract) only when creating the file at idea, when writing a structure the step file doesn't spell out, or for status/list's derived-read rules.
...
| `git worktree` | code-touching steps, when config `worktrees: true` | Worktree dirs sit next to the repo dir: `<repo-dir>--<slug>` |
...
When config `worktrees: true` (recommended): code-touching steps (prototype, implement, review, local QA, push) run in git worktrees so the user's main checkout stays untouched; docs-only steps need none. implement creates them (recorded in state.yaml), later steps reuse them from state.yaml, complete removes them; prototype manages its own temporary ones.
...
Derive "next" from the first non-done pipeline step (skipped optionals don't block). A missing slug folder means the workflow completed (complete deletes it) or never existed - say which is likelier and stop.
