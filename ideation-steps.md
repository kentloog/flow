...
Derive the questions from idea.md's Open Questions (let the user amend), then investigate the codebase and external sources. Run independent investigations as background subagents where the harness has them, and keep working meanwhile. Return key files, patterns, and extension points with a `path:line` anchor per code claim. Use primary sources for external claims and include a URL per claim. Combine the findings into `research.md`; update state.yaml.
...
3. **Route by question type:**
   - **"Does this logic / state model feel right?"** - isolate the logic in a portable pure module (a pure reducer, an explicit state machine, or a small set of pure functions) behind a thin throwaway driver. Prefer a single HTML file with free-play controls and a few guided walkthroughs that a non-developer can drive; a terminal script is fine when the audience is only the developer. The driver imports the module and calls into it; nothing flows the other direction. When the question is answered, the validated core lifts into the real code on its own.
